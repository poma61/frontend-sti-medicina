...
const loadDataIterator = () => {
    ...
    setTimeout(async () => {
        const tema = new Tema({ area: { name: 'pediatria' } })
        ...
        <v-data-iterator :items="data" :items-per-page="9" :search="search_item"
            ...
            <template v-slot:header>
                <v-text-field v-model="search_item" clearable density="comfortable" hide-details
                    placeholder="Buscar temas..." prepend-inner-icon="mdi-magnify" color="cyan-darken-1" class="pa-2" />
            </template>
                ...
                <v-container class="as__data-iterator" fluid>
                    <v-row justify="center">
                        <v-col v-for="(item, index) in items" :key="index" cols="auto" md="4">
                            ...
                            <v-card class="pa-2 d-flex justify-space-between flex-column" elevation="2" min-height="240px">
                                ...
                                <v-card-actions>
                                    <v-divider color="cyan-darken-1" opacity="0.4"></v-divider>
                                    <v-btn color="cyan-darken-1" variant="tonal" class="ma-1" @click="showTema(item.raw)">
                                        Estudiar tema
