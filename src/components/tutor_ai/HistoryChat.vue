    ...
    <v-app-bar elevation="0" app style="background-color: transparent;" >
        ...
        <v-tooltip text="Audio TutorAI">
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="ma-1" :icon="prop.p_generate_audio_status ? 'mdi-volume-high' : 'mdi-volume-off'"
                    color="indigo-darken-1" variant="elevated"  @click="emit('toGenerateAudioStatus')" />
            </template>
        </v-tooltip>

        <v-tooltip text="Ayuda TutorAI">
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="ma-1" icon="mdi-help" color="indigo-darken-1" variant="elevated"  @click="emit('toOpenHelpDialog')"  />
            </template>
        </v-tooltip>

        <v-tooltip text="Nuevo mensaje">
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" class="ma-1" icon="mdi-plus" color="indigo-darken-1" variant="elevated"  @click="emit('toNewChat')" />
            </template>
        </v-tooltip>
        <v-tooltip text="Chats">
            <template v-slot:activator="{ props }">
                <v-btn v-bind="props" :icon="drawer? 'mdi-menu-right':'mdi-menu-left'" class="ma-1" @click.stop="drawer = !drawer"
                    variant="elevated" color="indigo-darken-1" />
            </template>
        </v-tooltip>
